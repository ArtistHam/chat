...
class Application {
  ...
  attachRoutes() {
    ...
    app.get("/rooms/:roomId/messages", this.getMessageHandler.bind(this));
    app.post(
  ...
  getMessageHandler(req, res) {
    let room = this.manager.getById(req.params.roomId);
    if(!room) {
      res.status(404).json({});
    } else {
      let messageJson = room.messages.map(message => message.toJson());
      let response = {
  ...
  postMessageHandler(req, res){
    ...
    if (!room) {
      ...
      let message = romm.postMessage(req.body.body, req.body.username);
      let response = {
